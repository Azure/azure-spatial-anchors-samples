...
QuadRenderer::QuadRenderer(const std::shared_ptr<DX::DeviceResources>& deviceResources, bool tagAlong) 
...
void QuadRenderer::PositionHologram(SpatialCoordinateSystem const& coordinateSystem)
{
...
// This function uses a SpatialPointerPose to position the world-locked hologram
// two meters in front of the user's heading.
void QuadRenderer::UpdateHologramPosition(SpatialCoordinateSystem const& currentCoordinateSystem, SpatialPointerPose const& pointerPose, const DX::StepTimer& timer)
{
    if (m_coordinateSystem)
    {
        ...
        {
            const float3 offset = float3(0.0f, 0.13f, 0.f);
            m_position = m_lastPosition = translation(transform.Value()) + offset;
        }
    }
    else if (m_tagAlong)
    {
...
void QuadRenderer::CreateDeviceDependentResources()
{
    ...
    auto pixelShaderFileData = shaderFileData.GetTexturePixel();
    winrt::check_hresult(
    ...
    const CD3D11_BUFFER_DESC vertexBufferDesc(sizeof(VertexPositionColorTex) * static_cast<UINT>(quadVertices.size()), D3D11_BIND_VERTEX_BUFFER);
    winrt::check_hresult(
        m_deviceResources->GetD3DDevice()->CreateBuffer(
            &vertexBufferDesc,
            &vertexBufferData,
    ...
    const CD3D11_BUFFER_DESC indexBufferDesc(sizeof(unsigned short) * static_cast<UINT>(quadIndices.size()), D3D11_BIND_INDEX_BUFFER);
    winrt::check_hresult(
        m_deviceResources->GetD3DDevice()->CreateBuffer(
            &indexBufferDesc,
            &indexBufferData,
