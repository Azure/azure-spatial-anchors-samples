...
void TextRenderer::RenderTextOffscreen(const std::wstring& title, const std::wstring& status, const std::wstring& msg)
{
    if (title == m_lastTitle && msg == m_lastMsg && status == m_lastStatus)
    {
        return;
    }
    m_lastTitle = title;
    ...
    auto text = title + L"\n" + status + L"\n" + msg;
    DWRITE_TEXT_RANGE titleRange = { 0, static_cast<UINT32>(title.size() )};
    DWRITE_TEXT_RANGE statusRange = { static_cast<UINT32>(title.size()) + 1, static_cast<UINT32>(status.size()) };
    DWRITE_TEXT_RANGE msgRange = { static_cast<UINT32>(title.size()) + 2 + static_cast<UINT32>(status.size()), static_cast<UINT32>(msg.size()) };
    ...
    winrt::check_hresult(textLayout->SetFontWeight(DWRITE_FONT_WEIGHT_BOLD, titleRange));
    winrt::check_hresult(textLayout->SetFontSize(80.0f, statusRange));
...
void TextRenderer::CreateDeviceDependentResources()
{
    ...
    winrt::check_hresult(m_d2dRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White), &m_whiteBrush));
